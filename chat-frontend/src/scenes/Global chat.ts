...
import { sendMessage, subscribe, unsubscribeFromTopic, sendMessageByTopic } from '../actions'
import { requestPermission } from '../getToken';



export default class GlobalChat extends Phaser.Scene {
    ...
    create(): void {
        ...
        mainPanel.on('send-message', async (message: string) => {
            ...
            await sendMessageByTopic(message);
        });
